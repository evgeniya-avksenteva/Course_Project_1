...
def mock_dependencies(mocker) -> None:
    ...
    mocker.patch(
        "os.getenv", side_effect=lambda key: "fake_api_key" if key in {"API_KEY_CURRENCY", "API_KEY_STOCKS"} else None
    )


def test_main(mock_file: None, mock_dependencies: None) -> None:
