...
def read_transaction_excel(file_excel: str) -> list:
    """Функция принимает путь до xlsx файла
    и создает список словарей с транзакциями"""
    try:
...
#    result_excel = read_transaction_excel("../data/operations.xlsx")
...
def greeting() -> str:
    """Функция определяет время суток и возвращает приветствие
...
def get_cards_data(transactions: List[Dict]) -> List[Dict]:
    """Функция создает словарь с ключами номеров карт и
    в значения добавляет сумму трат и сумму кэшбека"""
    card_data = {}
    for transaction in transactions:
...
def get_exchange_rates(currencies: List[str], api_key_currency: Any) -> List[Dict]:
    ...
    for currency in currencies:
        ...
        if response.status_code == 200:
            data = response.json()
            ruble_cost = round(data["conversion_rates"]["RUB"], 2)
...
def get_stocks_cost(companies: List[str], api_key_stocks: Any) -> List[Dict]:
    """Функция принимает список кодов компаний и возвращает словарь
    со стоимостью акций каждой переданной компании"""
    stocks_cost = []
